...
var messages = [{text: 'first text', owner: 'Tomek'}, {text:'secomd text', owner: 'Janek'}];
var users = [{}];
...
function sendToken(user, res) {
  var token = jwt.sign(user.id, 'pass');
  res.json({firstName: user.firstName, token});
}

function sendAuthError(res) {
  return res.json({succcess: false, message: 'email or password incorrect'});
}
